...
export function InvitationForm(props) {
  ...
  const [validateEmails, setValidateEmail] = useState(false);
  ...
  const validateMultipleEmailsCommaSeparated = value => {
    ...
    for (let i = 0; i < result.length; i++)
      if (!validateEmail(result[i])) return setValidateEmail(false);
                ...
                >
                  {selectTextArea && selectTextArea ? (
                    <div className="textarea-input">
                      ...
                      />
                      {validateEmails && validateEmails ? "invalid email":""}
