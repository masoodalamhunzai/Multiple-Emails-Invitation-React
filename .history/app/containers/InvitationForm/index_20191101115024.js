...
export function InvitationForm(props) {
  ...
  const handleChangeTextArea = e => {
    ...
    validateEmailsCommaSeparated(textArea);
  };
  ...
  const validateEmailsCommaSeparated = value => {
    let commaResult = value && value.split(',');
    console.log(commaResult)
    for (let i = 0; i < commaResult.length; i++)
      if (!multipleEmailValidate(commaResult[i])) return false;
    return true;
  };
